...
T_USER                 : U S E R; 
T_DBPROPERTIES         : D B P R P O P E R T I E S ;
...
T_DISTRIBUTED          : D I S T R I B U T E D ;
T_MACRO                : M A C R O ;
T_SHOW                 : S H O W ;
...
L_ID        : L_ID_PART ('[' (L_ID_PART | '\'' .*? '\'' | '"' .*? '"') ']')?    // Identifier
            ;
...
//Y - tinyint, S - smallint, L - bigint
L_INT       : L_DIGIT+ (Y | S | L)?;                                               // Integer
L_DEC       : L_DIGIT+ '.' ~'.' L_DIGIT* B? D? F?                      // Decimal number
            | '.' L_DIGIT+
...
fragment
L_ID_PART  :
             [a-zA-Z] ([a-zA-Z] | [0-9] | '_' | '[' | ']')*                           // Identifier part
            | '$' '{' .*? '}'
            | ('_' | '@' | '#' | '$') ([a-zA-Z] | [0-9] | '_' | T_DIV | '@' | '#' | '$')+     // (at least one char must follow special char)
            | '`' .*? '`'
            ;
fragment
