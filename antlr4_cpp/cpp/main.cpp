...
#include "./cpp_parser/error_verbose_listener.hpp"

using namespace antlr4;

void ParseFile(const char* filename);
void ParseString(std::string &sql);
static void _ParseString(CPP14Parser &parser, CommonTokenStream &tokens);
...
static void _ParseString(CPP14Parser &parser, CommonTokenStream &tokens) {
  auto cpp_ctx = parser.translationunit();
  std::cout << "sql: " << tokens.getText() << std::endl;
  std::cout << ((tree::ParseTree*)(cpp_ctx))->toStringTree(&parser) << std::endl;
}
